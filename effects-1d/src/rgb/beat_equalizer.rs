...
fn hash(bucket: u16, beat: i32) -> f32 {
    ...
    x = x.wrapping_mul(1_103_515_245).wrapping_add(beat as u32);
    x ^= x >> 15;
...
impl BeatBasedEffect for BeatEqualizer {
    ...
    fn render_frame(
        ...
        let envelope = if beat_phase < 0.15 {
            beat_phase / 0.15
        } else {
            (1.0 - (beat_phase - 0.15) / 0.85).clamp(0.0, 1.0)
        };
        for bucket in 0..self.buckets {
            ...
            let level = (target * envelope * phase_bias).clamp(0.0, 1.0);
            let hue = 360.0 * f32::from(bucket) / f32::from(self.buckets) + beat.current as f32 * 12.0;
            let color = color::hsv8(hue, 0.9, level);

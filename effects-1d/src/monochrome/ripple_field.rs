...
impl Ripple {
    fn new(beat: BeatInfo, rng: &mut EffectRng) -> Self {
        Self {
            position: rng.gen_range(0.08..0.92),
            start_beat: beat.current,
            start_fractional: beat.fractional,
            duration: rng.gen_range(3.0..7.0),
            max_radius: rng.gen_range(0.18..0.48),
        }
    }

    fn render(self, beat: BeatInfo, framebuffer: &mut dyn FrameBufferRef<color::Monochrome>) -> Option<Self> {
        ...
        framebuffer.draw_smooth(self.position - radius - width, self.position - radius + width, color, BlendMode::Add);
        framebuffer.draw_smooth(self.position + radius - width, self.position + radius + width, color, BlendMode::Add);
        Some(self)
    }
}
...
impl BeatBasedEffect for RippleField {
    ...
    fn render_frame(
        ...
        for ripple in &mut self.ripples {
            *ripple = ripple.take().and_then(|ripple| ripple.render(beat, framebuffer));
        }
        if beat.is_new_beat {
            for ripple in &mut self.ripples {
                if ripple.is_none() {
